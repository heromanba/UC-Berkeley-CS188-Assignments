...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if actions == ():
            ...
            QOfState = util.Counter()
            for action in actions:
                QOfState.update({(state, action): self.getQValue(state, action)})
            maxValue = -100000000000
            bestActions = []
            for i in QOfState.items():
                if i[1] > maxValue:
                    maxValue = i[1]
                    bestActions = [i[0][1]]
                if i[1] == maxValue:
    ...
    def update(self, state, action, nextState, reward):
        ...
        #print(self.episodesSoFar,action,self.computeValueFromQValues(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        self.weights.incrementAll(list(features.keys()),0)
